...
class Yaml(val input: ParserInput) extends Parser {
    ...
    def c_indicator = rule { c_sequence_entry | c_mapping_key | c_mapping_value |
      c_collect_value | c_sequence_start | c_sequence_end | c_mapping_start |
      c_mapping_end | c_comment | c_anchor | c_alias | c_tag | c_literal |
      c_block | c_single_quote |c_double_quote | c_directive | c_reserved
    }

    def c_flow_indicator = rule {
      c_collect_value | c_sequence_start |
      c_sequence_end | c_mapping_start | c_mapping_end
    }

    // line break characters [25-30]
    ...
    // white space characters [31-34]
    def s_tab   = c_tab
    def s_space = c_space
    def s_white = c_tab ++ c_space
    def ns_char = nb_char -- s_white

    // miscellaneous characters [35-40]
    ...
    def ns_esc_8_bit               = rule { CharPredicate('x') ~ 2.times(ns_hex_digit) }
    def ns_esc_16_bit              = rule { CharPredicate('u') ~ 4.times(ns_hex_digit) }
    def ns_esc_32_bit              = rule { CharPredicate('U') ~ 8.times(ns_hex_digit) }

    def ns_esc_char = rule { c_backslash ~ ( ns_esc_null | ns_esc_bell | ns_esc_backspace |
      ns_esc_horizontal_tab | ns_esc_line_feed | ns_esc_vertical_tab | ns_esc_form_feed |
      ns_esc_carriage_return | ns_esc_escape | ns_esc_space | ns_esc_double_quote | ns_esc_slash |
      ns_esc_backslash | ns_esc_next_line | ns_esc_non_breaking_space | ns_esc_line_seperator |
      ns_esc_paragraph_seperator | ns_esc_8_bit | ns_esc_16_bit | ns_esc_32_bit)
    }

    // indentation spaces [63-65]
    def ns_indent(n: Int) = rule { n.times(s_space) }
    def ns_indent_lt(n: Int) = rule { MATCH }
}
